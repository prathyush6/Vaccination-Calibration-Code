...
for k in K:
    for t in T:
       for l in L:
       	   ys[k, t, l] = m.addVar(vtype = GRB.BINARY, name = "ys("+str(k)+","+str(t)+","+str(l)+")")
           yie[k, t, l] = m.addVar(vtype = GRB.BINARY, name = "yie("+str(k)+","+str(t)+","+str(l)+")")

for k in K:
   for t in T: 
       m.addConstr( quicksum(ys[k,t,l]* pow(b, l) for l in L) - quicksum(xs[10*k+i,t] for i in ten) >= - 1000, name = "RC6["+str(k)+","+str(t)+"]")	
       m.addConstr( quicksum(ys[k,t,l]* pow(b, l) for l in L) <= quicksum(xs[10*k+i,t] for i in ten), "RC7["+str(k)+","+str(t)+"]")
       m.addConstr( quicksum(yie[k,t,l]* pow(b, l) for l in L) - quicksum(xie[10*k+i,t] for i in ten) >=  - 1000, name = "RC8["+str(k)+","+str(t)+"]")
       m.addConstr( quicksum(yie[k,t,l]* pow(b, l) for l in L) <= quicksum(xie[10*k+i,t] for i in ten), name = "RC9["+str(k)+","+str(t)+"]")
...
for i in R:
    for t in range(0,len(T)-1):
        k = int(i/10)
        m.addConstr(quicksum( ((theta[i][j]*beta)/Neff[j]) * quicksum( z[k,int(j/10),t,l,ll] * pow(b, l+ll) for l in L for ll in L) for j in R)/100.0 == xs[i,t] - xs[i,t+1] -xv[i,t] , name = "RC13["+str(i)+","+str(t)+"]")  
        m.addConstr(quicksum( ((theta[i][j] *beta)/Neff[j]) * quicksum( z[k,int(j/10),t,l,ll] * pow(b, l+ll) for l in L for ll in L) for j in R)/100.0 == -xe[i,t] + xe[i,t+1] + alpha *xe[i,t], name = "RC14["+str(i)+","+str(t)+"]") 
